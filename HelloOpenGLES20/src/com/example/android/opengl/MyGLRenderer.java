...
public class MyGLRenderer implements GLSurfaceView.Renderer {
    ...
    public void onSurfaceCreated(GL10 unused, EGLConfig config) {
        ...
        for (int i = 0; i < 3*numPts; i+=3) {
        	circle[i] = (radius)*((float)Math.cos(((float)(2*Math.PI*a/numPts))));//(float) (radius * Math.cos(i/4.0 * 2.0 * Math.PI));
        	circle[i+1] = (radius)*((float)Math.sin(((float)(2*Math.PI*a/numPts))));;//(float) (radius * Math.sin(i/4.0 * 2.0* Math.PI));
        	circle[i+2] = 1.0f;
        	a++;
        }
